...
def gradcheck(func, inputs, eps=1e-6, atol=1e-5, rtol=1e-3, raise_exception=True, check_sparse_nnz=False, nondet_tol=0.0):
    ...
    for i, o in enumerate(output):
        print("checking if output ",i, "requires grad")
        if not o.requires_grad:
            continue
        print("output ",i, " requires grad so we will calculate the jacobian for it")
        ...
        print("analytical jacobian has size ", len(analytical))
        print("numerical jacobian has size ", len(numerical))

        for j in range(len(analytical)):
            # torch.set_printoptions(profile="full")
            # print('Jacobian for output %d with respect to input %d,\n' 'numerical:%s\nanalytical:%s\n' % (i, j, numerical[j], analytical[j]))
            # torch.set_printoptions(profile="default")
            diff=numerical[j]-analytical[j]
            diff_norm=diff.norm()
            max_error=diff.abs().max()
            # print("numerical is ", numerical[j])
            # print("analytical is ", analytical[j])
            print("diff_norm is ", diff_norm)
            print("max absolute error is ", max_error)
            diff_max_abs_diff=diff.abs().max()
            # print("diff_maximum absolute difference is ", diff_max_abs_diff)
        ...
        for j, (a, n) in enumerate(zip(analytical, numerical)):
            ...
            if a.numel() != 0 or n.numel() != 0:
                if not torch.allclose(a, n, rtol, atol):
                    return fail_test("fail because jacobian mismatch")
